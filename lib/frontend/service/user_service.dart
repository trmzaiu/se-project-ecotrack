...
import '../../database/model/leaderboard.dart';
...
class UserService {
  ...
  Stream<Users?> getCurrentUser(String userId) {
    return _db.collection('users').doc(userId).snapshots().map((snapshot) {
      ...
      return Users.fromMap(data, userId);
    });
  }
  ...
  Future<List<Users>> getUsersByIds(List<String> userIds) async {
    ...
    for (int i = 0; i < userIds.length; i += batchSize) {
      ...
      for (var doc in querySnapshot.docs) {
        final data = doc.data();
        data['userId'] = doc.id;
        users.add(Users.fromMap(data, data['userId']));
      }
    }
